...
def predfromCH(HSSsize, gridscale=False, userAmb=None, plotdefAmb=True, t0=None, omniname=None, shadezones=True, plotstart=None, plotstop=None, obsstart=None, obsstop=None, savename=None):
    ...
    for i in range(6):
        if omniname:
            axes[axi[i]].plot(obstimes, np.abs(omniSW[i])/normFactors[i], 'k', lw=3)
        if plotdefAmb:
            axes[axi[i]].plot(MHtimes, np.abs(data[:,i]*ambSWdef[i])/normFactors[i], '--', color='b', lw=3)
        if userAmb:
            axes[axi[i]].plot(MHtimes, np.abs(data[:,i]*userAmb[i])/normFactors[i], color= 'r', lw=3)
        
        ylims = axes[axi[i]].get_ylim()    
        if shadezones:
            # add shading
            for j in range(5):
                axes[axi[i]].fill_between([zbounds[j], zbounds[j+1]], [ylims[0], ylims[0]], y2=[ylims[1], ylims[1]],color = fills[j], alpha=0.15, zorder=0)  
        axes[axi[i]].set_ylabel(labels[i])
        
        if obsstart:
            axes[axi[i]].plot([obsstart, obsstart], [ylims[0], ylims[1]], 'k--')
        if obsstop:
            axes[axi[i]].plot([obsstop, obsstop], [ylims[0], ylims[1]], 'k--')
            
        axes[axi[i]].set_ylim(ylims)
...
# Sep 2016  
predfromCH(34, gridscale=True, userAmb=[8, 330, 2.5, 3.5, 50000], t0='2016/09/16 13:32', omniname='omniSep2016.dat', plotstart='2016/09/18 13:30', plotstop='2016/09/23 18:30', obsstart='2016/09/19 04:00', obsstop='2016/09/22 22:00', savename='MHpred_Sep16.png')

# July 2017 
predfromCH(52, gridscale=True, userAmb=[6, 330, 2, 2, 40000], t0='2017/07/05 19:32', omniname='omniJul2017.dat', plotstart='2017/07/07 12:00', plotstop='2017/07/14 12:00', obsstart='2017/07/08 23:00', obsstop='2017/07/12 18:00', savename='MHpred_Jul17.png')

# Aug 2017 
predfromCH(50, gridscale=True, userAmb=[4.5, 350, 2, 3, 40000], t0='2017/07/31 19:59', omniname='omniAug2017.dat', plotstart='2017/08/02 12:00', plotstop='2017/08/09 12:00', obsstart='2017/08/03 10:00', obsstop='2017/08/08 04:00', savename='MHpred_Aug17.png')

# Nov 2017 
predfromCH(26, gridscale=True, userAmb=[10, 330, 1, 1.5, 30000], t0='2017/11/17 13:49', omniname='omniNov2017.dat', plotstart='2017/11/20 02:00', plotstop='2017/11/24 20:00', obsstart='2017/11/20 12:00', obsstop='2017/11/24 13:00', savename='MHpred_Nov17.png')